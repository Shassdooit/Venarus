...
if (document.documentElement.clientWidth > 960) {
  ...
  const figura = document.querySelector('.hero__images');
  let switchCount = 0;
...
if (document.documentElement.clientWidth <= 960) {
  let f1 = document.querySelector('.hero__image_one');
  let f2 = document.querySelector('.hero__image_two');
  let f3 = document.querySelector('.hero__image_three');
...
if (iconMenu) {
  const disc = document.querySelector(".hero__disclaimer");
...
function showSlides() {
  ...
  slidesWrapper.onmouseout = () => timer = setTimeout(showSlides, 3000);
};
...
const closeModal = () => {
  modal.forEach((el) => {
    const Iframe = el.querySelectorAll('iframe')[0];
